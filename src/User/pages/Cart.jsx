...
export default function UserCart() {
  ...
  const totalAmount =
    dishCharges + shopCharges + deliveryCharges + taxCharges || 0;
  ...
  const handleCheckout = async () => {
    try {
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          try {
            ...
            if (paymentResponse.data.status) {
              ...
              const clearCartResponse = await axiosInstance.post('/api/payments/clear-cart', {}, {
                headers: { Authorization: `Bearer ${token}` }
              });
  
              console.log('Clear Cart Response:', clearCartResponse.data); // Log the response
  
              // Clear the cart items from the state and local storage
              setCartItems([]);
              setShopCartItems([]);
              localStorage.removeItem('cartItems'); // Clear cart from local storage if applicable
              localStorage.removeItem('shopCartItems'); // Clear shop cart from local storage if applicable
            } else {
        ...
        prefill: {
          name: 'DINE AT HOME',
          email: 'dhammashila025@gmail.com',
          contact: '9324486349',
        },
        theme: {
          color: '#3399cc',
